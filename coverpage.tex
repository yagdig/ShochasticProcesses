...
% 封面页,自《代数学方法》卷一调整, 由主档引入.

\setCJKfamilyfont{coverfont}{Source Han Sans CN Heavy}	% 设置书名字体
\setCJKfamilyfont{cover-author-font}{Source Han Sans CN}	% 设置作者字体
...
\begin{titlepage}\begin{tikzpicture}[remember picture, overlay, pencildraw/.style={
		...
		\node[anchor=center] (title) at ([xshift=20em, yshift=-10em] current page.north west)
		{ \fontsize{45}{45}\CJKfamily{coverfont}随机过程回顾};

		\node[anchor=center,scale=2.1] (volume) at ([yshift=-5em] title.center) {\fontsize{40}{5}\CJKfamily{coverfont}Review of Stochastic Process};%

		\node[anchor=west] (author) at ([yshift=-4em, xshift=4pt] volume.south west) {\fontsize{20}{20}\CJKfamily{cover-author-font}龙\ 汉};
		\node[anchor=west] at ([xshift=2em] author.east) {\fontsize{20}{20}\CJKfamily{cover-author-font}讲授};

		\node[anchor=west] (author) at ([yshift=-7em, xshift=4pt] volume.south west) {\fontsize{20}{20}\CJKfamily{cover-author-font}杨\ 鼎};
		\node[anchor=west] at ([xshift=2em] author.east) {\fontsize{20}{20}\CJKfamily{cover-author-font}整理};

		\draw[line width=2pt, color=black!70!gray] ([xshift=-2em, yshift=4em] author.north west) -- ++(30em,0);
